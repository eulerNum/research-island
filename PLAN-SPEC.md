...
| 행동 | 트리거 | 효과 |
|------|--------|------|
| 하이라이트 | 💡 아이콘 클릭 | 맵에서 관련 다리/도로 glow 효과 |
| 상세 보기 | 논문 텍스트 클릭 | 가운데 DetailPanel에 논문 정보 표시 |
...
### Phase 3-A: 사이드바 리디자인 (미구현)
- [ ] 접이식 사이드바 (아이콘 ↔ 펼침 전환)
- [ ] Save/Load 최상단 배치
- [ ] Bridges 섹션 제거
- [ ] Settings → 홈페이지 이동
- [ ] 맵 요약 숫자 표시 (섬, 다리, 논문, 갭)
- [ ] 논문 연도별/저널별 그룹핑 & 필터
- [ ] 논문 통계 (연도 분포)
- [ ] 논문 인터랙션 분리 (💡하이라이트 / 클릭=상세 / 드래그=배치)
- [ ] 드래그&드롭 영역 확대
- [ ] 갭 메모 → 다리 포스트잇 날아가기 애니메이션

### Phase 3-B: 섬 조망도 인터랙션 강화 (미구현)
- [ ] 섬 hover=하이라이트, 클릭=포커스이동+도시펼침, 더블클릭=내부뷰 진입
- [ ] 섬 펼침 시 내부 도시 노출 (섬이 커지면서)
